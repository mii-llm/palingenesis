...
class Pipeline:
    ...
    def __init__(
        ...
        routes: dict[str, TeacherRoute],
        weights: PublishedWeights,
        chat_template_kwargs: dict,
        score_micro_seqs: int,
    ...
    def generate(self, prompts: list[list[int]], max_new_tokens: list[int], temperature: float):
        ...
        return rollouts, {
            "time/sync": sync,
            "time/rollout": t2 - t1,
            "time/wake_sleep": (t1 - t0) + (t3 - t2),
            "rollout_tokens": tokens,
            "rollout_tok_s": tokens / max(t2 - t1, 1e-9),
        }
    ...
    def run(self, requests: list[Request], temperature: float) -> Batch:
        ...
        with self._exclusive():
                ...
                if completion:
                    samples.append(
                        Sample(
                            request,
                            completion,
                            rollout.logprobs[: len(completion)],
            ...
            for name, route in self.routes.items():
                ...
                if group:
                    scores = route.teacher.score(
                        [s.view for s in group],
                        top_k=route.top_k,
                        keep_hidden=route.keep_hidden,
                        micro_seqs=self.score_micro_seqs,
...
class Orchestrator:
    ...
    def __init__(self, pipeline: Pipeline, draw: Callable[[], list[Request]], temperature: float, max_staleness: int):
    ...
    def next(self, version: int) -> Batch:
        ...
        while True:
            ...
            if version - item.version > self.max_staleness:
                ...
                logger.warning(
                    "dropped a batch %d versions old (max_staleness %d)", version - item.version, self.max_staleness
                )
                continue
            return item
