...
def end_of_turn_id(tok, chat_template_kwargs: dict | None = None) -> int:
    ...
    text = tok.apply_chat_template(
        [{"role": "user", "content": "hi"}, {"role": "assistant", "content": marker}],
        tokenize=False,
...
def sample_teacher(
    logp: Tensor, rounds: int, temperature: float, generator: torch.Generator | None = None
    ...
    def score(
        ...
        for start in range(0, len(order), micro_seqs):
            ...
            positions = completion_positions(
                [views[i].prompt_len for i in chunk], [views[i].completion_len for i in chunk], ids.shape[1]
            )
            ...
            with torch.autocast(ids.device.type, dtype=torch.bfloat16, enabled=ids.is_cuda):
                targets = torch.tensor(
                    [t for i in chunk for t in views[i].input_ids[views[i].prompt_len :]], device=ids.device
                )
            ...
            for i in chunk:
                ...
                results[i] = TeacherScores(
                    token_lp[sl],
                    topk_ids[sl] if top_k else None,
...
class VLLMTeacher:
    ...
    def score(
        ...
        choices = self.server.complete(
            [v.input_ids for v in views], max_tokens=1, temperature=1.0, prompt_logprobs=max(top_k, 1)
        )
    ...
    def _parse(view: TeacherView, prompt_logprobs: list, top_k: int) -> TeacherScores:
        ...
        return TeacherScores(
            torch.tensor(token_lp, dtype=torch.float32),
