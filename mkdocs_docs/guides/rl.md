...
Every step logs what to watch: `reward` and `rewards/*`, `policy/abs_ratio_dev` and `policy/mismatch_k3` (sampler/trainer agreement; a rise precedes collapse), `policy/clipped`, `policy/seq_masked`, `policy/entropy`, `groups/zero_variance`, `truncated`, `skipped`, and the time split (`time/rollout`, `time/train`). Rollout diagnostics: `engine/seqs_mean` (sequences decoding at once), `engine/tail_s` (time with under a quarter of the peak left), `engine/prefix_hit` and `engine/prefill_tokens`, `groups/continued`. `logging.dump_trajectories: N` writes every Nth batch (turns, tool calls and outputs, rewards, per-turn timing, zero-variance groups too) and the engine's concurrency timeline to `<output_dir>/rollouts/`: read it before tuning anything.
...
- **Overlap.** `rollout.max_staleness: 1` generates, runs tools and scores step k+1 while step k trains. Groups still generating when a batch is full keep going and join the next batch (partial rollouts; new weights load between engine steps, and the importance ratio uses the log-probs each token was sampled with), so the engine does not drain waiting for a batch's slowest groups.
- **Budgets decide multi-turn speed.** A group is complete when its slowest rollout is, so a batch waits for the longest serial chain of turns. Size `rollout.max_completion_tokens` from what successful trajectories use: on agentic GSM8K with Qwen3.5-0.8B, halving it (cutting 1% of correct trajectories) made steps 1.7x faster. Measured and not worth it there: MTP speculative decoding (`rollout.vllm_args: {speculative_config: ...}`, slower at ~130 sequences in flight), smaller prefill chunks (no change), a bf16 recurrent state (`{mamba_ssm_cache_dtype: bfloat16}`: a few % faster, 2.5x the sampler/trainer mismatch), and 64 groups per batch instead of 32 (the same trajectories per second: decoding is saturated). A profiled Qwen3.5-0.8B decode step at 128 sequences is 10 ms, 95% GPU kernels: the linear-attention state update (3 ms), attention (1.9 ms), GEMMs (2 ms) and the vocabulary softmax (0.7 ms).
